...
func PerformMove(chessBoard *Board.ChessBoard, move Move) { //function that'll perform given move on given chessBoard, we need a pointer to the board as we wanna change the color to move
	...
	if piece, ok := chessBoard.GetPieceAtBoardPosition(move.EndPos.X, move.EndPos.Y); ok == true { //check if there is a piece at the move destination pos if so, remove it, effectifly taking it with the piece that'll be moved
